...
# Setup dashboard.service via systemd
if [ ! -f "/etc/systemd/system/dashboard.service" ]
then
sudo mv /etc/consul.d/consul/scripts/dashboard-startup.sh /home/ec2-user/startup.sh
sudo chmod +x /home/ec2-user/startup.sh
sudo cp /etc/consul.d/consul/systemd/dashboard.service /etc/systemd/system/dashboard.service
sudo systemctl daemon-reload
